<?php
/**
 * The template for displaying for front page
 *
...
<main class="flex flex-col h-svh">
    ...
    <div class="flex flex-row gap-x-4 gap-y-8 justify-between px-[2.8vmax] w-full max-w-[65%] xs:max-w-[25%] absolute top-1/2 -translate-y-1/2 text-xl">
        <?php if(!empty($menu)): ?>
        <?php foreach ($menu as $item): ?>
            <a href="<?=$item['link']['url']?>" target="_self"><?=$item['link']['title']?></a>
        <?php endforeach;?>
        <?php endif; ?>
    </div>
    ...
    <div class="flex justify-between items-center flex-row xs:flex-col lg:flex-row gap-4 px-[2.8vmax] pb-[2.2vmax] xs:pb-[1vmax]">
        ...
        <div class="xs:w-full">
            <div class="flex flex-col xs:flex-row justify-between items-end xs:items-center">
                ...
                <p class="normal-case text-gray-20 font-sans align-right xs:order-2 order-3">© 2025. Symbiotica</p>
